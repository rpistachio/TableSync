...
exports.main = async (event, context) => {
  ...
  if (mode === 'image') {
    ...
    return { code: 400, message: `不支持的模式: ${mode}，当前支持 image 和 link` };
  }
  ...
  if (!apiKey) {
    return {
      code: 500,
      message: 'MOONSHOT_API_KEY 未配置，请在 secret-config.json 中配置',
    };
  }
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════
    if (mode === 'link') {
      return await handleLinkImport({ url, apiKey, textModel, timings, startTime });
    }

    // ═══════════════════════════════════════════════════════════
    // mode === 'image': 截图导入流程（原有逻辑）
    // ═══════════════════════════════════════════════════════════
