...
> **维护者**: TableSync 团队  
> **v1.11 变更**: 云数据库新增 users 集合设计（4.6.1，OpenID 主键、family_id）；utils/tracker.js 通用埋点上报、7 个核心事件 + basket_add/schedule_preview_view，投篮与统筹预览触发时自动记录；9.8 埋点实施检查清单补充当前实施状态。  
...
> **设备上限**：各设备槽位上限由 `kitchenConfig` 唯一推导，算法通过 `computeDeviceLimits(kitchenConfig)` 得到；不单独存储。占灶设备仅 wok / stove_long / pot 及「无独立电蒸锅时的 steamer」；hasSteamer 为 true 时 steamer 不占灶；air_fryer、rice_cooker、oven 永不占灶。详见 `docs/design-kitchen-devices-adaptation.md`。
>
...
| `users` | 用户档案（OpenID 主键、家庭关联） | `_openid` (唯一), `family_id` | 随用户增长 |

#### 4.6.1 users 集合 (用户档案)

用于小程序云开发环境下以 **OpenID 为主键** 的用户记录，支持家庭组（family_id）等扩展字段。

| 字段 | 类型 | 必填 | 约束 | 默认值 | 说明 |
|------|------|------|------|--------|------|
| `_id` | String | 可选 | 若云函数写入可设为 openid | 自动 | 文档主键；不传时自动生成，查询时以 `_openid` 作为用户唯一键 |
| `_openid` | String | 自动 | 云开发按当前用户自动注入 | — | **逻辑主键**：同一用户在同一环境内唯一，用于权限与查询 |
| `family_id` | String | 否 | 同一家庭内一致，如 UUID 或邀请码 | `null` | 家庭/户组 ID，用于多成员共享菜单、购物清单等 |
| `nickname` | String | 否 | — | `null` | 昵称（可选，来自 getUserProfile 或自定义） |
| `avatar_url` | String | 否 | 头像 URL | `null` | 头像（可选） |
| `createTime` | Date | 否 | 首次创建时写入 | — | 记录创建时间 |
| `updateTime` | Date | 否 | 每次更新时写入 | — | 最后更新时间 |

**设计说明**:

- **OpenID 作为主键**: 微信云开发在每条记录上自动注入 `_openid`，仅当前用户可读写自己的文档。以 `_openid` 作为用户唯一标识即可，无需在客户端传 openid；若在云函数内希望用 openid 作为 `_id` 便于 `doc('users/' + openid).get()`，可在云函数中显式指定 `_id: openid` 写入。
- **family_id**: 用于「家庭」维度功能（如多人共享今日菜单、购物清单）。同户成员写入相同 `family_id`，可由云函数或后端按 `family_id` 聚合查询。
- **权限建议**: 集合权限设为「仅创建者可读写」，这样用户只能读写自己的 `users` 文档；若需按 `family_id` 跨用户读，需在云函数内用管理端或服务端权限查询。

**示例文档** (云开发自动带 `_openid`):

```javascript
{
  "_id": "xxx",           // 可选：云函数中可用 openid 作为 _id
  "_openid": "oABC123...", // 自动注入
  "family_id": "fam_uuid_001",
  "nickname": "小明",
  "avatar_url": "https://...",
  "createTime": "2026-02-09T00:00:00.000Z",
  "updateTime": "2026-02-09T12:00:00.000Z"
}
```
...
- **通用上报**: `miniprogram/utils/tracker.js` 已封装 `trackEvent(eventId, properties)`，支持 7 个核心事件 ID + `basket_add`、`schedule_preview_view`；尊重 `tracking_opt_out`，失败时入队、`app.onLaunch` 时 `flushTrackingQueue()` 补报。
- **投篮自动记录**: import 页（保存后加入灵感篮）、myRecipes 页（心形入篮）、scan 页（冰箱推荐自动入篮）触发时上报 `basket_add`。
- **统筹预览自动记录**: mix 页 `_updateSchedulePreview()` 在有待选菜时上报 `schedule_preview_view`（每页一次，onShow 重置）。
- **steps 页内 7 个核心事件**: 仍待在 `steps.js` 中按下列清单接入（会话状态、onLoad/onHide/onShow/onUnload、markCompleted/checkParallelCompletion/triggerFallback 等处调用 `tracker.trackEvent(...)`）。

```
□ 1. steps.js 新增 _trackingState 对象和 generateSessionId() 函数
✅ 2. 使用 utils/tracker.js 统一 trackEvent()（已实现）
...
✅ 投篮/统筹预览 已接入 basket_add、schedule_preview_view
□ 10. 微信后台配置自定义分析事件 (7 个 + basket_add + schedule_preview_view)
